...
const fresnelAtmosphereMaterial = (color: THREE.Color, intensity: number, power: number) =>
...
const fitCameraToSphere = (camera: THREE.PerspectiveCamera, radius: number, margin = 1.22) => {
...
const Globe3D = () => {
  ...
  useEffect(() => {
    ...
    const sphereGeo = (radius: number) => new THREE.SphereGeometry(radius, segments, segments);
    ...
    interface PingData { ring: THREE.Mesh; phase: number }
    ...
    interface ArcData { line: THREE.Line; prog: number; speed: number; pts: THREE.Vector3[]; mat: THREE.LineBasicMaterial }
    const arcs: ArcData[] = [];
    ...
    const animate = () => {
      ...
      pings.forEach(({ ring, phase }) => {
        ...
        (ring.material as THREE.MeshBasicMaterial).opacity = (1 - p) * 0.72;
      });

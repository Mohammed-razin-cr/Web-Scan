...
const sessionResumption = (n) =>
  n === 1 ? '✅ Tickets' : n === 2 ? '✅ Tickets (no client cert)' : '❌ Disabled';

const compression = (n) => (n ? '⚠️ Supported (CRIME risk)' : '✅ Disabled');
...
const TlsSecurityAuditCard = (props
  ...
  const d = ep.details;
  const protocols = (d.protocols || []).map((p) => `${p.name} ${p.version}`).join(', ');
  return (
